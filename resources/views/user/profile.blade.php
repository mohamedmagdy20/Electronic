...
<section class="profile container pt-3 pb-3">
    <div class="text-center">
        ...
        <div class="profile-details">
            <div class="profile-email">{{auth('client')->user()->name}}</div>
            <div class="profile-email">{{auth('client')->user()->email}}</div>
            <div class="profile-email">{{auth('client')->user()->phone}}</div>
            <div class="profile-email">{{auth('client')->user()->address}}</div>
    ...
    <table class="table mt-5">
        ...
        <tbody>
        @if (! $data->isEmpty())
          @foreach ($data['order_details'] as $index => $item )
          <tr class="text-center">
